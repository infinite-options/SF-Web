...
function subPrice(items) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    var number = parseInt(
      window.localStorage.getItem(JSON.stringify(items[i]))
    );
    total += price * number;
  }
  return total;
...
function Cart() {
  ...
  function stripePay() {
    ...
    for (var i = 0; i < allItemsHolder.length; i++) {
      var packOgItems = {
        qty: parseInt(
          window.localStorage.getItem(JSON.stringify(allItemsHolder[i]))
        ),
  ...
  var finalPrice = (
    Math.floor(
      (tax * subprice + shippingCost + subprice - couponValue * subprice) * 100
    ) / 100
  ).toFixed(2);
